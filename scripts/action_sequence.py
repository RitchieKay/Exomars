...
class PCF:
    ...
    def __init__(self, pcf):
        ...
        for line in f:
            ...
            if len(records[2]) > 0:
                self.pid_param[int(records[2])] = records[1]
        f.close()

    def get_param(self, pid):
        return self.pid_param[pid]

class ActionSequenceProcessor:

    def __init__(self):

        self.critical_acqs = [61, 63, 117, 118, 119, 120, 121, 115]
        self.params = PCF('pcf.dat')
        self.pids = {1: 'PUS', 2:'PM', 3:'TC', 4: 'TM', 5:'EQ', 6:'FDIR', 7:'PUS Inactive', 8:'PM Inactive', 9:'SWIN Inactive', 10:'SWIN', 11:'AOCS', 12:'SADM', 13:'APM', 14:'PL', 15:'TH', 16:'VM'}
        ...
        self.acseq_ids[69]	= 'ACSEQ_ENABLE_SPIN_PMON_NOT_IN_AEROBRAKING'
        ...
        self.acseq_ids[143]	= 'ACSEQ_L1_TGO_BATTERY_SWITCH_1_RECOVERY'
        self.acseq_ids[144]	= 'ACSEQ_ENTER_INIT_PREPARATION'
        self.acseq_ids[145]	= 'ACSEQ_ENTER_INIT_PROP_VENTING'
        self.acseq_ids[146]	= 'ACSEQ_EDM_RTPU_POWER_CYCLE'
        ...
        self.acseq_ids[155]	= 'ACSEQ_ENABLE_ACSEQ_FOR_FAIL_SAFE_AFTER_FAILOP'
        self.acseq_ids[156]	= 'ACSEQ_DISABLE_ACSEQ_FOR_FAIL_SAFE_AFTER_FAILOP'
        ...
        self.acseq_ids[161]	= 'ACSEQ_SET_ALL_CU_RCT_TO_FAILED_IN_SAT_CONF'
        self.acseq_ids[162]	= 'ACSEQ_ENABLE_STR_L1_FDIR'
        self.acseq_ids[163]	= 'ACSEQ_SWAP_ALL_RCT'
        ...
        self.acseq_ids[255]	= 'ACSEQ_RECONFIGURE_TGO_HEATER_GROUPS_PART_2'
        self.acseq_ids[256]	= 'ACSEQ_ENTER_SAFE_1_MODIFICATION_BLOCK'
        self.acseq_ids[257]	= 'ACSEQ_ENTER_SAFE_A_MODIFICATION_BLOCK'
        ...
        self.acseq_ids[261]	= 'ACSEQ_ENTER_SAFE_2_MODIFICATION_BLOCK'
        self.acseq_ids[262]	= 'ACSEQ_ENTER_SAFE_2_MODIFICATION_BLOCK'
        self.acseq_ids[263]	= 'ACSEQ_LAUNCH_TO_INIT_CONFIGURE_FDIR'
        self.acseq_ids[264]	= 'ACSEQ_RECONFIGURE_TGO_HEATER_GROUPS_PART_1'
        self.acseq_ids[265]	= 'ACSEQ_PROP_CLOSE_ALL_LATCH_VALVES'
        self.acseq_ids[266]	= 'ACSEQ_L1_SAPR_PZ_1_RECOVERY'
        self.acseq_ids[267]	= 'ACSEQ_L1_SAPR_PZ_2_RECOVERY'
        self.acseq_ids[268]	= 'ACSEQ_L1_SAPR_PZ_3_RECOVERY'
        self.acseq_ids[269]	= 'ACSEQ_L1_SAPR_PZ_4_RECOVERY'
        self.acseq_ids[270]	= 'ACSEQ_L1_SAPR_MZ_1_RECOVERY'
        self.acseq_ids[271]	= 'ACSEQ_L1_SAPR_MZ_2_RECOVERY'
        self.acseq_ids[272]	= 'ACSEQ_L1_SAPR_MZ_3_RECOVERY'
        self.acseq_ids[273]	= 'ACSEQ_L1_SAPR_MZ_4_RECOVERY'
        self.acseq_ids[274]	= 'ACSEQ_ENABLE_EDM_TCS_SLOT_PART_1'
        self.acseq_ids[275]	= 'ACSEQ_ENABLE_EDM_TCS_SLOT_PART_2'
        self.acseq_ids[276]	= 'ACSEQ_ENABLE_EDM_TCS_SLOT_PART_3'
        self.acseq_ids[277]	= 'ACSEQ_ENABLE_PZ_SAPR_FDIR'
        self.acseq_ids[278]	= 'ACSEQ_ENABLE_MZ_SAPR_FDIR'
...
#
        ...
        for line in f:
            ...
            if m1: 
                if len(bytes) > 0:
                    ...
                    print '=========================================================================================================================================='
                    length = self.process_data(bytes)
                    if int(ID) in self.critical_acqs:
                        self.write_patch(int(ID), bytes, length)
                    bytes = []
    ...
    def s2k_obsm(self, filename):
        ...
        return self.process_data(bytes)

    def write_patch(self, id, data, length):

        f = open(self.acseq_ids[id] + '.s2k', 'w')
        f.write('DOMAIN=0\n')
        f.write('ID='+str(id)+ '\n')
        f.write('VERSION=0.0\n')
        f.write('TYPE=PATCH\n')
        f.write('DESCRIPTION=' + self.acseq_ids[id] + '\n')
        f.write('SOURCE=\n')
        f.write('CREATIONDATE=\n')
        f.write('DEVICE=006_SMUA\n')
        f.write('STARTADDR=00000000\n')
        f.write('ENDADDR=%(l)08X\n'% {'l':length})
        f.write('LENGTH=%(l)08X\n'% {'l':length})
        f.write('UNIT=1\n')

        lines = length / 15

        for i in range(lines):
            f.write('START=%(s)08X,COUNT=F,DATA=' % {'s':i*15})
            for j in range (i*15, i*15 + 15):
                f.write('%(d)02X' % {'d':data[j]})
            f.write('\n')

        remaining = length - (lines * 15)
        if remaining > 0:
            f.write('START=%(s)08X,COUNT=%(c)X,DATA=' % {'c':remaining, 's':lines*15})            
            for j in range(lines * 15, length):
                f.write('%(d)02X' % {'d':data[j]})
            f.write('\n')
    ...
    def process_cmd(self, data):
        ...
        if type == 142 and subtype in [1,2]:
        ...
        elif type == 128 and subtype in [128,131]:
            if subtype == 128:
                fid =  self.int_32(data[10:14])
                mode = self.int_32(data[14:18])

                if pid == 'PM' and fid == 1:
                    mode = ['TEST', 'LAUNCH', 'INIT', 'MAN_C', 'ROUT', 'MAN_R', 'MAN_A', 'SCR', 'SAFE_1', 'SAFE_2', 'SAFE_A'][mode]
                elif (pid == 'SADM' or pid == 'APM') and fid == 0:
                    mode = ['MANUAL', 'SAFE', 'CRUISE'][mode]
                elif pid == 'AOCS' and fid == 0:
                    mode = ['SBM', 'SAM', 'NOMP', 'NOMR', 'OCM', 'AEBM'][mode]
                text +=  'Mode change ' + pid + ' ' + str(mode)
            elif subtype == 131:
            ...
            if subtype == 128:
                N = self.int_16(data[10:12])
                id = self.int_32(data[12:16])
                text += 'Set the function parameter value ' + self.params.get_param(id)
                    

        elif type == 132 and subtype in [2,3]:
            N = self.int_16(data[10:12])
            fids = []
            for i in range(N):
                fids.append(str(self.int_16(data[12+i:14+i])))
            if subtype == 2:
                text += 'Enable ACSEQ: '
            elif subtype == 3:
                text += 'Disable ACSEQ: '
